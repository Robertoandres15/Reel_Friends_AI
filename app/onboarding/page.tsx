...
import { User, ArrowRight, MapPin, Loader2 } from "lucide-react";
...
export default function OnboardingPage() {
    ...
    const getUser = async () => {
      try {
        ...
        if (sessionError || !session) {
          console.log(
            "[v0] No valid session found in onboarding:",
    ...
    if (cleanedZip.length === 5) {
      ...
      try {
        ...
        const response = await fetch(
          `https://api.zippopotam.us/us/${cleanedZip}`,
        );
        if (response.ok) {
  ...
  const handleCityChange = async (value: string) => {
    ...
    if (value.length >= 2) {
      ...
      const filtered = commonCities.filter((location) =>
  ...
  const handleProfileSetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Profile verification failed. The profile may belong to a different user.",
        );
      }

      console.log(
        "[v0] Profile setup successful and verified for user:",
        user.id,
      );
              ...
              <form onSubmit={handleProfileSetup} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    This will be your unique identifier and display name
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2 relative">
                      ...
                      <Input
                        ...
                        onFocus={() =>
                          city.length >= 2 && setShowSuggestions(true)
                        }
                        onBlur={() =>
        ...
        {step === 2 && (
          <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-white">Welcome to Castd!</CardTitle>
              <CardDescription className="text-slate-300">
                Your profile has been created successfully. You're ready to
                start discovering movies with friends.
              </CardDescription>
            </CardHeader>
