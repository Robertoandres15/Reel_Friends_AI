...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      console.log(
        "[v0] SMS invite: Supabase environment variables not available, using mock response",
      );
    ...
    const inviteMessage =
      message ||
      `${currentUser?.display_name || "Your friend"} invited you to join Castd! Download the app and connect with friends over movies and shows. [App Link]`;
