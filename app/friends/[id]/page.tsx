...
export default async function FriendProfilePage({
  ...
  const { data: friendship } = await supabase
    ...
    .select("*")
    .or(
      `and(user_id.eq.${user.id},friend_id.eq.${params.id}),and(user_id.eq.${params.id},friend_id.eq.${user.id})`,
    )
    .eq("status", "accepted")
    .single();
  ...
  const recommendationsList = friendLists?.find(
    (list) => list.type === "recommendations",
  );
  const wishlist = friendLists?.find((list) => list.type === "wishlist");
  const otherLists =
    friendLists?.filter(
      (list) => list.type !== "recommendations" && list.type !== "wishlist",
    ) || [];
    ...
    <div className="min-h-screen pt-[var(--safe-area-inset-top)] pb-[var(--safe-area-inset-bottom)] bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="container mx-auto px-4 py-8 pb-20 md:pb-8">
        <div className="max-w-4xl mx-auto">
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-6">
                <Avatar className="h-24 w-24">
                  <AvatarImage
                    src={friendProfile.avatar_url || "/placeholder.svg"}
                  />
                ...
                <div className="flex-1">
                  ...
                  {friendProfile.bio && (
                    <p className="text-slate-300 mb-4">{friendProfile.bio}</p>
                  )}
          ...
          {recommendationsList &&
            recommendationsList.list_items &&
            recommendationsList.list_items.length > 0 && (
          ...
          {wishlist &&
            wishlist.list_items &&
            wishlist.list_items.length > 0 && (
              <Card className="bg-slate-800/80 border-slate-600 backdrop-blur-sm mb-6">
                <CardHeader>
                  <CardTitle className="text-slate-200">
                    Wishlist ({wishlist.list_items.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card className="bg-slate-800/80 border-slate-600 backdrop-blur-sm">
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-slate-400">
                    {friendProfile.display_name || friendProfile.username}{" "}
                    hasn't shared any public lists yet.
                  </p>
                </CardContent>
              </Card>
            )}
